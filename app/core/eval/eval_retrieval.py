...
class RetrievalEvaluator:
    ...
    def evaluate(self, query_id: str, result: RetrievalResult) -> Dict[str, float]:
        ...
        metrics.update(
            self._compute_modality_metrics(
                "imagery", retrieved_imagery, relevant_imagery
            )
        )
        ...
        retrieved_text = []
        if result.tweets:
            retrieved_text.extend([str(t["tweet_id"]) for t in result.tweets])
        if result.calls:
            retrieved_text.extend([str(c["record_id"]) for c in result.calls])
        # Note: FEMA and Claims IDs might need consistent formatting with ground truth

        relevant_text = set(gt.get("text", []))
        metrics.update(
            self._compute_modality_metrics("text", retrieved_text, relevant_text)
        )

        return metrics

    def _compute_modality_metrics(
        self, prefix: str, retrieved: List[str], relevant: Set[str]
    ) -> Dict[str, float]:
        if not relevant:
