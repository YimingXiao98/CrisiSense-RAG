...
Set `HARVEY_RETRIEVER_MODE=legacy` (default). Switch to `hybrid` once the new retriever/recorder pipeline is indexed.
...
## Optional: Hybrid Retriever Setup
...
```
3. Set `HARVEY_RETRIEVER_MODE=hybrid` (and optionally `HARVEY_ENABLE_RERANKER=true`) before launching the API.

Until these assets exist the server stays on the legacy retriever, so local development remains lightweight.
